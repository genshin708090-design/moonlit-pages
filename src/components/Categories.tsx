  ...
  return (
    <section id="categories" className="mx-auto max-w-7xl px-6 py-20">
      <div className="mb-12 text-center">
        <div className="mb-3 text-sm uppercase tracking-[0.3em] text-violet-400">Explore by Genre</div>
        <h2 className="text-4xl md:text-5xl font-black">Browse <span className="bg-gradient-to-r from-violet-400 to-purple-300 bg-clip-text text-transparent">Categories</span></h2>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {categories.map((cat) => (
            ...
            key={cat.name}
            href="#library"
            className={`group relative overflow-hidden rounded-2xl border ${cat.border} bg-gradient-to-br ${cat.color} p-5 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-violet-500/10 backdrop-blur-sm`}
          >
