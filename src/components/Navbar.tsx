    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${scrolled ? "bg-[#05050f]/95 backdrop-blur-2xl border-b border-white/[0.06] py-3 shadow-xl shadow-black/20" : "bg-transparent py-6"}`}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6">
        ...
        <div className="hidden md:flex items-center gap-1">
          {["Library", "Categories", "Featured", "About"].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="px-4 py-2 text-sm text-zinc-400 hover:text-white hover:bg-white/5 rounded-xl transition-all duration-200">{item}</a>
          ))}
        </div>

        <div className="hidden md:flex items-center gap-3">
          <a href="https://buymeacoffee.com/genshin7086" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-xl border border-yellow-500/30 bg-yellow-500/10 px-4 py-2 text-sm font-semibold text-yellow-300 hover:bg-yellow-500/20 transition-all duration-300">
            Support Us
          </a>
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden w-9 h-9 flex items-center justify-center rounded-xl border border-white/10 text-zinc-400 hover:text-white transition">
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-white/5 bg-[#05050f]/98 backdrop-blur-2xl px-6 py-5 space-y-1">
          {["Library", "Categories", "Featured", "About"].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm text-zinc-300 hover:text-white hover:bg-white/5 rounded-xl transition">{item}</a>
          ))}
