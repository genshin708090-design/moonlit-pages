Set-Content src/app/admin/page.tsx @'
"use client"
...
export default function AdminPage() {
  ...
  async function handleSubmit() {
    ...
    if (editingBook) {
      ...
      else { setMessage("Book updated successfully!"); cancelEdit(); fetchBooks() }
    } else {
      const { error } = await supabase.from("books").insert({ title, author, genre, description, cover_url: coverUrl, epub_url: epubUrl })
      setLoading(false)
      if (error) { setMessage("Error: " + error.message) } 
      else { setMessage("Book added successfully!"); setTitle(""); setAuthor(""); setDescription(""); setCoverUrl(""); setEpubUrl(""); fetchBooks() }
    }
  }
  ...
  if (checking) {
    return <main className="flex min-h-screen items-center justify-center bg-[#070711] text-zinc-500">Checking auth...</main>
  }
    ...
    <main className="min-h-screen bg-[#070711] text-white">
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-10">
        <div className="mb-10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href="/" className="rounded-xl border border-white/10 px-4 py-2 text-sm text-zinc-400 hover:text-violet-300 hover:border-violet-500/30 transition">
              View Site
            </a>
            <button onClick={handleLogout} className="rounded-xl border border-red-500/20 px-4 py-2 text-sm text-red-400 hover:border-red-400/40 hover:bg-red-500/10 transition">
              Sign Out
            </button>
          </div>
        </div>
          ...
          <div className="rounded-3xl border border-white/5 bg-[#0f0f1e] p-8">
            {editingBook && (
              <div className="mb-6 flex items-center justify-between rounded-2xl border border-violet-500/20 bg-violet-500/10 px-5 py-3">
                ...
                <button onClick={cancelEdit} className="text-xs text-zinc-500 hover:text-white transition">Cancel Edit</button>
              </div>
            )}
            ...
            <div className="grid md:grid-cols-2 gap-5">
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description" className={`${inputClass} min-h-[120px] resize-none md:col-span-1`} />
            </div>
              ...
              books.map((book) => (
                <div key={book.id} className="flex items-center gap-4 rounded-2xl border border-white/5 bg-[#0f0f1e] p-4 hover:border-violet-500/20 transition">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => startEdit(book)} className="rounded-xl border border-white/10 px-3 py-2 text-xs text-zinc-400 hover:text-violet-300 hover:border-violet-500/30 transition">
                      ✏️ Edit
                    </button>
                    <button onClick={() => deleteBook(book.id)} className="rounded-xl border border-red-500/20 px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 transition">
                      🗑️
                    </button>
                  </div>
                </div>
